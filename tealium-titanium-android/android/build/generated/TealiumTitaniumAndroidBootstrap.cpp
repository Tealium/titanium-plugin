...
#define TAG "com.tealium.titaniumandroid"

using namespace v8;

static Persistent<Object> bindingCache;

static void TealiumTitaniumAndroid_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TealiumTitaniumAndroid.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TealiumTitaniumAndroidBindings::lookupGeneratedInit(
...
static void TealiumTitaniumAndroid_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TealiumTitaniumAndroid_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TealiumTitaniumAndroid_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TealiumTitaniumAndroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TealiumTitaniumAndroidBinding = {
	"com.tealium.titaniumandroid",
	TealiumTitaniumAndroid_init,
	TealiumTitaniumAndroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_tealium_titaniumandroid_TealiumTitaniumAndroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.tealium.titaniumandroid", &TealiumTitaniumAndroidBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TealiumTitaniumAndroidBindings::lookupGeneratedInit));
}
