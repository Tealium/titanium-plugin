function Tealium$Cache() {
    ...
    db.execute("CREATE TABLE IF NOT EXISTS event (id INTEGER PRIMARY KEY, raw_js TEXT, event TEXT, timestamp INTEGER, evaluated INTEGER);");
    ...
    this.storeEvent = function(event, evaluated) {
        var db = Ti.Database.open(DB_NAME);
        db.execute("INSERT INTO event (raw_js, event, timestamp, evaluated) VALUES (?, ?, ?, ?)", Tealium.Util.convertEventToJS(event), JSON.stringify(event), new Date().getTime(), evaluated ? 1 : 0);
        db.close();
        queueCount++;
    };
    ...
    this.loadEvents = function() {
        ...
        while (c.isValidRow()) {
            loaded.push({
                ...
                evaluated: 1 == c.field(2, Titanium.Database.FIELD_TYPE_INT)
            });
            c.next();
        }
        c.close();
...
"undefined" == typeof Tealium$Cache;